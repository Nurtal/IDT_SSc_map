...
Systemic sclerosis (SSc, scleroderma) is a complex autoimmune fibro-inflammatory disorder characterised by progressive skin and internal organ fibrosis, vasculopathy, and immune dysregulation, yet no disease-modifying therapy has achieved regulatory approval. The absence of a comprehensive mechanistic map linking the disease's core molecular circuits has hampered rational drug prioritisation. Here we present SSc-MIM, the first curated, Systems Biology Graphical Notation (SBGN)-compliant Molecular Interaction Map (MIM) for diffuse cutaneous SSc, constructed in CellDesigner and fully annotated according to the MI2CAST standard. The integrated map encompasses 526 molecular species across 260 reactions distributed over 17 subcellular and tissue-level compartments, organised into four biologically coherent modules: type-I interferon and cGAS–STING signalling (M1), TGF-β–driven fibroblast-to-myofibroblast transition (M2), endothelial-to-mesenchymal transition and vasculopathy (M3), and IL-6/IL-4/IL-13/B-cell crosstalk (M4). In total, 85 SSc-specific curated reactions are supported by 355 unique PubMed identifiers and annotated with ECO evidence codes. The map was paired with a published single-cell RNA-sequencing atlas of SSc skin biopsies (Tabib et al., 2021; GSE138669) to produce a transcriptomic overlay stratifying disease-relevant fibroblast populations. Network analysis identified 38 functional communities, with SMAD3–SMAD4 (hub score 13.42), TGFB1 (9.09), and NICD1 (9.46) as the three highest-connectivity hubs. Cross-referencing against the Drug–Gene Interaction database revealed 21 SSc-relevant drug–target interactions, including fresolimumab (TGFB1), anifrolumab (IFNAR1/2), tocilizumab (IL6R), and brontictuzumab (NOTCH1). SSc-MIM provides a community resource for hypothesis generation, computational modelling, and translational stratification in scleroderma research.
...
The Tabib et al. (2021) scRNA-seq dataset from SSc skin biopsies (GSE138669) was used to overlay transcript-level differential expression onto the map. Cell-type-specific expression profiles for fibroblast subpopulations (SFRP2/DPP4⁺ papillary fibroblasts and FMO1/LSP1⁺ reticular fibroblasts) and their SSc-altered counterparts were quantified and mapped onto corresponding MIM species using HGNC-based symbol matching (`scripts/tabib_fetcher.py`, `scripts/overlay_synthetic.py`). Species with absent map coverage but significant SSc differential expression were flagged for prospective curation. The overlay was visualised as a two-panel heatmap stratified by SSc subtype (diffuse vs. limited) and fibroblast population.
...
### 3.2 Single-Cell Transcriptomic Overlay and Subtype Stratification

Mapping the Tabib et al. (2021) scRNA-seq atlas onto SSc-MIM identified 312 map species with detectable expression in at least one skin fibroblast subpopulation (60% map coverage). Of these, 97 species showed statistically significant differential expression (|log₂FC| > 0.5, adjusted p < 0.05) between SSc and healthy control skin, and 43 were differentially expressed between the diffuse and limited SSc subtypes (Figure 2).

The SFRP2/DPP4⁺ papillary fibroblast subpopulation, previously characterised as the primary source of SSc-specific COL1A1 and ACTA2 upregulation (Tabib et al., 2021), showed the greatest overlap with M2 hub species: TGFB1, SMAD3, COL1A1, ACTA2, and YAP1 were all upregulated in this subpopulation relative to controls. In contrast, the FMO1/LSP1⁺ reticular fibroblasts, which retain a more homeostatic anti-fibrotic programme, showed preferential upregulation of SFRP2 (a Wnt antagonist and negative regulator of fibrosis) and lower SMAD3 activation scores. Endothelial cells from SSc skin exhibited significant NOTCH1 and CDH5 dysregulation consistent with active EndoMT, linking single-cell observations to the M3 module hub species NICD1 (hub score 9.46) and CDH5 (hub score 4.87).

At the subtype level, the type-I IFN module (M1) was disproportionately active in dcSSc compared with limited cutaneous SSc, with the ISGF3 transcriptional complex (hub score 8.95) and the ISG signature output node (hub score 8.91) among the most differentially expressed complexes. This pattern aligns with the known enrichment of the IFN gene expression signature in dcSSc patients with anti-topoisomerase I autoantibodies (Assassi et al., 2010; Rice et al., 2017), and positions M1 as a potential stratification biomarker distinguishing treatment-responsive subgroups.
...
The integration of the Tabib et al. (2021) scRNA-seq atlas with SSc-MIM revealed a 60% coverage rate for the map's molecular species in the skin fibroblast transcriptome — a notably high figure that validates the biological relevance of the curated species selection. The differential enrichment of M1 hub species in dcSSc compared with limited SSc, and their preferential expression in the SFRP2/DPP4⁺ fibroblast subpopulation, is consistent with the emerging model that this subpopulation is the primary driver of the progressive skin score in early dcSSc (Tabib et al., 2021; Hinchcliff et al., 2023). From a stratification perspective, these findings suggest that M1-module hub scores could serve as a computational biomarker of IFN-high dcSSc, a subgroup most likely to respond to type-I IFN blockade with anifrolumab — a hypothesis now being tested in the ongoing PRISM-SSc trial.
